    ...
    try {
        ...
        if (data.nfts) { 
            const cleaned = data.nfts.map(n => ({
                ...n,
                display_artist: n.creator || (n.collection ? n.collection.split(' by ')[1] : null) || "Unknown Artist",
                high_res: n.image_url || n.display_image_url
            }));
...
function renderAll() {
    ...
    if (mode === 'snap') {
        ...
        Object.keys(groups).sort(() => Math.random() - 0.5).forEach(k => {
            ...
            items.forEach((n, idx) => {
                ...
                s.innerHTML = `
                    ...
                    <button class="quick-share" onclick="event.stopPropagation(); share('${n.opensea_url}')"><i data-lucide="share-2"></i></button>
                    <img src="${n.high_res}" onload="this.previousElementSibling.previousElementSibling.previousElementSibling.style.display='none'">`;
        ...
        list.forEach(n => {
            ...
            c.innerHTML = `<div class="loader-ring"></div><img src="${n.high_res}" onload="this.previousElementSibling.style.display='none'">`;
        ...
        m.innerHTML = `
            <div class="modal-body">
                <div class="modal-img-container">
                    <div class="loader-ring"></div>
                    <img src="${n.image_url || n.display_image_url}" id="modalMainImg" onload="this.previousElementSibling.style.display='none'">
                    ${isTwoStep ? '<p class="tap-hint">TAP FOR DETAILS</p>' : ''}
                </div>
                <div class="modal-text-content">
                    <h2 style="font-weight:900; font-size: 24px;">${n.name || 'UNTITLED'}</h2>
                    <p style="opacity:0.5; font-size:12px; letter-spacing:1px; text-transform:uppercase; margin-bottom: 20px;">${n.collection || ''}</p>
                    <p style="line-height:1.6; font-size:15px; opacity:0.8; margin-bottom: 30px;">${n.description || 'No description available.'}</p>
                    <a href="${n.opensea_url}" target="_blank" style="display:block; padding:18px; background:white; color:black; text-align:center; border-radius:12px; font-weight:900; text-decoration:none;">OPENSEA</a>
                </div>
            </div>
        `;
        const wrap = m.querySelector('.modal-img-container');
        if (isTwoStep) wrap.onclick = () => modalContent.classList.toggle('show-details');
...
gallery.onscroll = () => {
    ...
    if (document.documentElement.getAttribute('data-view') === 'grid' && cur > 400) bttBtn.classList.remove('hidden'); else bttBtn.classList.add('hidden');
    lastScrollY = cur;
...
document.getElementById('themeToggle').onclick = () => {
    ...
    document.getElementById('themeToggle').innerHTML = `<i data-lucide="${t === 'dark' ? 'sun' : 'moon'}"></i>`;
    lucide.createIcons();
};
